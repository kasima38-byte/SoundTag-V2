...
export default function ListenScreen({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!status.granted) {
        Alert.alert('Permission needed', 'soundTag needs microphone access to identify music.');
      }
    })();

    const animations = [
      makePulse(bar1, 0.4, 1, 500),
      makePulse(bar2, 0.6, 0.9, 650),
      makePulse(bar3, 0.3, 0.8, 550),
      makePulse(bar4, 0.7, 1, 480),
      makePulse(bar5, 0.4, 0.9, 620),
      makePulse(bar6, 0.2, 0.6, 700),
    ];
  ...
  const handleMicPress = async () => {
    ...
    try {
      ...
      if (data.found) {
        navigation.navigate('Result', { song: data.song });
      } else {
        Alert.alert('No match found', "We couldn't identify that song. Try again with clearer audio.");
      }
    } catch (error) {
      setIsListening(false);
  ...
  const barHeight = (val, base) => val.interpolate({
    inputRange: [0, 1],
    outputRange: [base * 0.2, base],
  });
  ...
  return (
    <LinearGradient colors={['#0D0710', '#150A1F', '#0D0710']} style={styles.container}>
      ...
      <Text style={styles.heading}>{isListening ? 'Listening...' : 'Tap to identify music'}</Text>
      <Text style={styles.subheading}>
        {isListening ? 'Analyzing the sound around you' : "We'll listen and find the song"}
      </Text>
...
const styles = StyleSheet.create({
  ...
  outerRing: {
    width: 260, height: 260, borderRadius: 130, borderWidth: 1.5, borderColor: '#7B2CBF',
    alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(157, 78, 221, 0.06)',
  },
  micButton: {
    width: 70, height: 70, borderRadius: 35, backgroundColor: '#9D4EDD',
    alignItems: 'center', justifyContent: 'center',
  },
  micButtonActive: { backgroundColor: '#7B2CBF' },
  bottomNav: {
    flexDirection: 'row', justifyContent: 'space-around',
    paddingTop: 16, paddingBottom: 32, borderTopWidth: 1, borderTopColor: '#1E1626',
  },
